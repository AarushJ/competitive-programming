...
int main(){
	...
	lp(i, n){
		cin >> a >> b;
		a--; b--;		// because my graph is zero indexed
		g[a].pb(b);
		g[b].pb(a);
	}
	...
	cin >> q;	// number of queries of the form (a,b) asking for LCA of vertices and b
	while(q--){
		cin >> a >> b;
		a--; b--;				// because my graph is zero indexed 
		int lca = LCA(a,b) + 1; // because my graph is zero indexed
		cout << lca << "\n";
	}
