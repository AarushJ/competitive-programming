...
int main(){
	...
	//cin.tie(NULL);
	ll lo,hi,beg,end,l,r,t,n,m,e,a,b,p,q,h,f,w,k;
    ///////////// START FROM HERE ////////////////////
	cin >> n >> k >> p;
	string s;
	...
	lp(i,p){
		...
		if(s[i] == '!'){
			...
			ll nlo = lo-1, nhi = hi-1;
			mp[lastWindowSum]--;
			if(mp[lastWindowSum]==0)	mp.erase(lastWindowSum);
			
			lastWindowSum += arr[hi-k];
			lastWindowSum -= arr[hi];
			
			firstWindowSum += arr[nlo];
			firstWindowSum -= arr[lo+k-1];
			mp[firstWindowSum]++;
