	...
	while(t--){
		...
		for(ll i = 0 ; i<n ; i++){
			...
			for(ll j = 1 ; j<m, i+j<n ; j++){
				num[i][j] = (num[i][j-1]*10) + (str[i+j]-'0');
			}
		}
		...
		for(ll i = 0 ; i<m ; i++){
			dp[0][i].insert(num[0][i]);
		}		
		for(ll i = 1 ; i <= y ; i++){
			for(ll j = 1 ; j<n ; j++){
				for(ll k = j-1 ; k >=0 && k >= j-m ; k--){
 					lpit(it,dp[i-1][k]){
						dp[i][j].insert(__gcd(*it, num[k+1][j-k-1]));
					}
				}
			}
		}
		...
		ll maxx = -1;
		for(ll i = x; i<=y ; i++){
			if( !dp[i][n-1].empty() ) maxx = max(maxx, *dp[i][n-1].rbegin());
		}
