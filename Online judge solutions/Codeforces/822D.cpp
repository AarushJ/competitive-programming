...
void spf_sieve(){	for(ll i=2 ; i<MAX ; i+=2)	spf[i] = 2;
	for(ll i=3 ; i<MAX ; i+=2){	if(!notPrime[i]){ spf[i] = i; for(ll j=i ; (j*i)<MAX ; j+=2){ if(!notPrime[i*j])	notPrime[i*j] = true , spf[i*j] = i; } } }
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int main(){
	ll t, n, m, v, p, k, a, b, c, d, e , type, x, y, z;
	// START HERE //		
	ll l,r;
	cin >> t >> l >> r;
	spf_sieve();
	...
	for(int i = 3 ; i<=r ; i++){ 
		f[i] = (f[i/spf[i]] + ((i*(spf[i]-1LL))>>1)) % mod;
	}
