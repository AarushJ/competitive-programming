...
struct node{
	ll l, r, type, cost;
	node(ll left, ll right, ll q, ll wt){
		l = left;
		r = right;
		type = q;
		cost = wt;
...
int main(){
	...
	lp(i, n){
		ll l, r , cost;
		cin >> l >> r >> cost;
		node tmp1(l, r, 0, cost);
		node tmp2(r, l, 1, cost);
		q.pb(tmp1);
		q.pb(tmp2);
	}

	sort(all(q), [](node a, node b){ if( a.l != b.l ){ return a.l < b.l ;} else return a.type < b.type; });
	ll ans = 4e10 + 10;
	lp(i, 2*n){
		ll days = q[i].r - q[i].l + 1;
		if( q[i].type ){
			// r
			bestCost[days] = min( bestCost[ days ], q[i].cost);
		}else{
			// l 
			if( days < x )
				ans = min( ans, q[i].cost + bestCost[x - days]); 
		}
	}
