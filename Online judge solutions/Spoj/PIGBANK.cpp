...
int main(){
	...
	while(t--){
		...
		w = f-e;
		cin >> n;
		ll  p[n+1], wt[n+1], dp[w+1];	
		lp(i,n)	cin >> p[i+1] >> wt[i+1];
		p[0] = 0, wt[0] = 0;
		//lp(i,n)	cout << p[i+1] << " " << wt[i+1] << "\n";
		
		lp(i,w+1)	dp[i] = -1;	// sentinel
		dp[0] = 0;
		
		for(ll i=1 ; i<=w ; i++){
			for(ll j=1 ; j<=n ; j++){
				if(wt[j] <= i && dp[i-wt[j]] != -1 ){
					if(dp[i]==-1)	dp[i] = p[j] + dp[i-wt[j]];
					else{
						dp[i] = min( dp[i] , p[j] + dp[i-wt[j]] );
					}
				}			
			}
		}
		
		if(dp[w]==-1)	cout << "This is impossible.\n";
