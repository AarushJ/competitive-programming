/*
	AARUSH JUNEJA
	@vivace
	TOPICS : Depth first search, pigeon hole principle, counting
*/
...
vector<pair<ll,ll> > g[100005]; // graph
vll cnt(100005),par(100005);	// cnt[i] => number of nodes in the subtree rooted at i
...
ll dfs(ll v,ll n){
	...
	lpit(it,g[v]){
		...
		if( !mark[u] ){
			par[u] = v;
			cnt[v] += (dfs(u,n));
			ans += (2*min( cnt[u] + 1, n - cnt[u] - 1)*it->ss);
		}
	}
	return (cnt[v] + 1);
}
