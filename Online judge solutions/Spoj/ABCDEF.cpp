...
int main(){
	...
	lp(i,n){
		lp(j,n){
			lp(k,n){
				l.pb( v[i] * v[j] + v[k]);	// all possible a*b + c
			}
		}
	}	
	...
	lp(i,n){
		lp(j,n){
			lp(k,n){
				...
				if( v[i] ){
					r.pb( (v[j] + v[k])*v[i] );	// all possible (f+e)*d such that d != 0
				}
	...
	lpit(lit,l){
		lb = lower_bound( all(r), *lit);
		ub = upper_bound( all(r), *lit);
		if( lb != r.end() && *lb == *lit ){
			if( ub == r.end())
				ans += (r.size()-1 - (lb-r.begin()) + 1);
			else
				ans += (ub-lb);
		}
	}
