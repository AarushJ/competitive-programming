/*
	AARUSH JUNEJA
	@vivace
	TOPIC : Matrix exponentiation, prefix sums
	
	LOGIC : Let S(n) = a(1) + a(2) + ... + a(n)
			Then S(n) = S(n-1) + a(n)	=> 1st recurrence
			Further => a(n) = a(n-1) + a(n-2) + ... + a(n-k) => 2nd recurrence
			Make 1 transition matrix only to handle both
			=> O((k^3)*log(n)) per test case
*/
...
ll solve(ll k,ll n,ll mod){
	...
	T = exp(T,n,k,mod);	// calculation of (T^n)%mod
	ll res = 0;
	lp(i,k)
...
int main(){
    ...
    while(t--){
    	...
    	cin >> m >> n >> p;
    	ll mod = p;
    	ll a = solve(k,n,mod), b = solve(k,m-1,mod);
		ll ans = a-b;
		if( ans < 0 )	ans += mod;
