...
int main(){
	...
	// dp[j] => is length of tower == j a winning state for Asen 
	bool dp[v[m-1] + 1];
	// dp[j] == true => winning state for Asen
	...
	lp(i,v[m-1]+1){
		...
		if( !dp[i+1] ) 	dp[i+1] = ( (dp[i]) ? false : true );	
		
		if( i + k > v[m-1] ){
			continue;
		}
		if( !dp[i+k] )	dp[i+k] = ( (dp[i]) ? false : true );
		
		if( i + l > v[m-1] ){
			continue;
		}
		if( !dp[i+l] )	dp[i+l] = ( (dp[i]) ? false : true );
	}	
	
	lp(i,m){
		if( dp[original[i]] )	cout << 'A';
		else cout << 'B';
	}
