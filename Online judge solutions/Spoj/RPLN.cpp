...
void build(ll pos, ll lo, ll hi){
	...
	build(l, lo, mid);
	build(r, mid+1, hi);
	seg[pos].val = min(seg[l].val,seg[r].val);
}	

ll query(ll pos, ll lo, ll hi, ll qlo, ll qhi){
	if( qlo > hi || qhi < lo || lo > hi )	return INT_MAX;
	if( qlo <= lo && qhi >= hi)	return seg[pos].val;
	ll mid = (lo+hi)>>1;
	ll l = pos+pos+1, r = l+1;
	ll left = query(l, lo, mid, qlo, qhi);
	ll right = query(r, mid+1, hi, qlo, qhi);
